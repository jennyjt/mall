...
public class GlobalDefaultExceptionHandler {
    ...
    public ResponseData defaultErrorHandler(HttpServletRequest request, HttpServletResponse response, Exception ex) throws Exception {
        ...
        String outputLog = "defaultErrorHandler [method={} url={} token={} code={} from={} to={} cost={}ms parameters=({})] msg=({})";
        String msg = ex.getCause() == null ? ex.getMessage(): ex.getCause().getMessage();
        ex.printStackTrace();
        logger.error(ex.getMessage());
        logger.error(outputLog, method, url, token, response.getStatus(), remoteAddr, requestDO.getLocalAddr(), String.format("%1$.3f", 0.00), queryString, msg);
    ...
    public ResponseData bindExceptionHandler(HttpServletRequest request, BindException ex) {
        ...
        if (ex.hasFieldErrors()) {
            ex.getFieldErrors().forEach(error ->
                    sb.append(String.format("参数 %s(%s) 错误：%s ", error.getField(), error.getRejectedValue(), error.getDefaultMessage()))
            );
        } else if (ex.hasGlobalErrors()) {
            ex.getGlobalErrors().forEach(error ->
                    sb.append(String.format("参数 %s 错误： %s ", error.getArguments(), error.getDefaultMessage()))
            );
        }
        String resultMsg = String.format("BindException: %s", sb.toString());
        responseData.setError(resultMsg);
